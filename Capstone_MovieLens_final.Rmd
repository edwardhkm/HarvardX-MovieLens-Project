...
# download.file("file:///C:/Work_proj/workspace_R/capstone_movielens/ml-10m.zip", dl)
# download.file("file:///d:/edward_ho/edx_data_science_cert/capstone/ml-10m.zip", dl)
...
rmse_results1 <- data.frame(method = "The average rating", RMSE = method_average)
rmse_results1
```
The RMSE is 1.0612.  This value is very far away from our target value.  
...
The RMSE is 0.9439.  This value is still very far away from our target value.  In order to improve our RMSE.  We are going to add the third attribute from the edx data to improve our algorithm.
...
method_movie_user_effects

rmse_results3 <- data.frame(method = "Movie_user_effects", RMSE = method_movie_user_effects)

```

The third method generated RMSE = 0.8653488.  We still have a room for improvement.  Therefore, we try to throw in additional attributes and hoping it will generate a RMSE below 0.865.
...
method_movie_user_year_effects

rmse_results4 <- data.frame(method = "Movie_user_year_effects", 
                            RMSE = method_movie_user_year_effects)

```
The improvement of employing movie released year attribute does not change RMSE much.
```{r check_improve, eval=TRUE}

improvement <- (method_movie_user_year_effects / method_movie_user_effects)
...
# movie released year effect
  b_y <- edx %>% left_join(b_i, by="movieId") %>% left_join(b_u, by="userId") %>% 
...
## Result
It shows improvement when we add more attribute into the lm algorithms.  However, it stops improving when the 3rd attribute added.  We are forced to review our algorithm and we need to apply regularization technique to improve the RMSE.  The final result of the RMSE is 0.8645625.

```{r my_result, echo=FALSE, include=FALSE, eval=TRUE}
# rmse_results1 = data.frame(method = "Just the average", RMSE = 1.0612018)
# rmse_results2 = data.frame(method = "Movie effects", RMSE = 0.9439087)
# rmse_results3 = data.frame(method = "Movie_user_effects", RMSE = 0.8653488)
# rmse_results4 = data.frame(method = "Movie_user_year_effects", RMSE = 0.8650034)
# rmse_results5 = data.frame(method = "Movie_reg_user_year_effects", RMSE =0.8645625)
...
```{r rmse_results}
rmse_all_results = rbind(rmse_results1, rmse_results2, rmse_results3, 
                         rmse_results3, rmse_results4, rmse_results5)
rmse_all_results
```

## Conclusion
The target of this project is to develop an algorithm that can achieve RMSE as small as possible.  Through the use of linear model with multiple attributes, we can arrive RMSE 0.8645625.
